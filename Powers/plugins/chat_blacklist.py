...
@Nikki.on_message(filters.command("blchat") & SUDOERS)
async def blacklist_chat(c: Nikki, m: Message):
...
@Nikki.on_message(filters.command("rmblchat") & SUDOERS)
async def unblacklist_chat(c: Nikki, m: Message):
    if len(m.text.split()) >= 2:
        chat_ids = m.text.split()[1:]
...
@Nikki.on_message(filters.command("blchatlist") & SUDOERS)
