...
@Nikki.on_message(filters.linked_channel)
async def antichanpin_cleanlinked(c: Nikki, m: Message):
    try:
        msg_id = m.id
...
@Nikki.on_message(filters.text & filters.group, group=5)
...
@Nikki.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)
async def gban_watcher(c: Nikki, m: Message):
...
@Nikki.on_message(filters.chat(BLACKLIST_CHATS))
async def bl_chats_watcher(c: Nikki, m: Message):
    from Powers import SUPPORT_GROUP
