...
from Powers import LOGGER, DEV_USERS, SUPPORT_GROUP
...
@Nikki.on_message(command(['floodaction','actionflood']) & admin_filter)
async def flood_action(c: Nikki, m: Message):
    Flood = Floods()
...
@Nikki.on_message(command(['isflood', 'flood']) & ~filters.bot)
async def flood_on_off(c: Nikki, m: Message):
    if m.chat.type == CT.PRIVATE:   
...
@Nikki.on_message(command(['setflood']) & ~filters.bot & admin_filter)
async def flood_set(c: Nikki, m: Message):
...
@Nikki.on_callback_query(filters.regex("^f_"))
async def callbacks(c: Nikki, q: CallbackQuery):
    data = q.data
    if data == "f_close":
    ...
    if user == DEV_USERS or user_status in [CMS.OWNER, CMS.ADMINISTRATOR]:
...
@Nikki.on_callback_query(filters.regex("^un_"))
async def reverse_callbacks(c: Nikki, q: CallbackQuery):
    data = q.data.split("_")
    action = data[1]
    ...
    if action == "mute":
        ...
        if not user.privileges.can_restrict_members and q.from_user.id == DEV_USERS:
            await q.answer(
                "You don't have enough permission to do this!\nStay in your limits!",
                show_alert=True,
            )
...
dic = {}
@Nikki.on_message(filters.all & ~filters.bot | ~filters.private, 10)
async def flood_watcher(c: Nikki, m: Message):
    c_id = m.chat.id
    ...
    if not is_flood or u_id == DEV_USERS:
...
__HELP__ = """
**Dᴇsᴄʀɪᴘᴛɪᴏɴ :**
ʏᴏᴜ ᴋɴᴏᴡ ʜᴏᴡ sᴏᴍᴇᴛɪᴍᴇs, ᴘᴇᴏᴘʟᴇ ᴊᴏɪɴ, sᴇɴᴅ 100 ᴍᴇssᴀɢᴇs, ᴀɴᴅ ʀᴜɪɴ ʏᴏᴜʀ ᴄʜᴀᴛ? ᴡɪᴛʜ ᴀɴᴛɪғʟᴏᴏᴅ, ᴛʜᴀᴛ ʜᴀᴘᴘᴇɴs ɴᴏ ᴍᴏʀᴇ!
ᴀɴᴛɪғʟᴏᴏᴅ ᴀʟʟᴏᴡs ʏᴏᴜ ᴛᴏ ᴛᴀᴋᴇ ᴀᴄᴛɪᴏɴ ᴏɴ ᴜsᴇʀs ᴛʜᴀᴛ sᴇɴᴅ ᴍᴏʀᴇ ᴛʜᴀɴ x ᴍᴇssᴀɢᴇs ɪɴ ᴀ ʀᴏᴡ. ᴀᴄᴛɪᴏɴs ᴀʀᴇ: ʙᴀɴ/ᴍᴜᴛᴇ/ᴋɪᴄᴋ/
──────────────────────── 
 
**Cᴏᴍᴍᴀɴᴅs :**
๏ /flood: ᴛᴏ ᴄʜᴇᴄᴋ ᴡᴇᴀᴛʜᴇʀ ᴛʜᴇ ɢʀᴏᴜᴘ ɪs ᴘʀᴏᴛᴇᴄᴛᴇᴅ ғʀᴏᴍ sᴘᴀᴍ ᴏʀ ɴᴏᴛ.
