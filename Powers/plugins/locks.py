...
async def lock_perm(c: Nikki, m: Message):
    ...
    if lock_type in ["msg", "text"]:
    ...
    elif lock_type in ["media", "photo", "pic"]:
        media = False
        ...
        pin = False
        perm = "pin"
    elif lock_type in ["links", "link", "url"]:
...
async def unlock_perm(c: Nikki, m: Message):
    ...
    if unlock_type in ["msg", "text"]:
    ...
    elif unlock_type in ["media", "photo", "pic"]:
        umedia = True
        ...
        return
    elif unlock_type in ["links", "url", "link"]:
...
async def is_approved_user(c:Nikki, m: Message):
    ...
    if m.forward_from:
        if m.from_user.id in ul or m.from_user.id in SUDO_LEVEL or m.from_user.id in admins_group or m.from_user.id == Nikki.id:
            return True
        return False
...
@Nikki.on_message(filters.group & ~filters.me,18)
